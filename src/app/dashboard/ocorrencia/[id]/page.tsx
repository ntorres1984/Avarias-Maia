...
export default function EditOccurrencePage() {
  ...
  const [originalObservacoes, setOriginalObservacoes] = useState<string | null>(null)
  const [originalDataEstado, setOriginalDataEstado] = useState<string | null>(null)
  const [originalDataEncerramento, setOriginalDataEncerramento] = useState<string | null>(null)
  const [originalDataReporte, setOriginalDataReporte] = useState<string | null>(null)

  async function loadHistory() {
    const { data } = await supabase
      .from('occurrence_history')
      .select('id, occurrence_id, estado_anterior, estado_novo, observacoes, data_alteracao')
      .eq('occurrence_id', id)
  ...
  async function loadOccurrence() {
    ...
    setOriginalObservacoes(item.observacoes)
    setOriginalDataEstado(item.data_estado)
    setOriginalDataEncerramento(item.data_encerramento)
  ...
  }, [dataReporte, dataEncerramento, slaDias, estado, originalDataReporte])
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if ((estado === 'Concluída' || estado === 'Encerrada') && !dataEncerramentoIso) {
      setErrorMessage('Para concluir ou encerrar, tem de existir data de fim.')
      setSaving(false)
      return
    }
